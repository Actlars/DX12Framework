...
namespace RHI
{
	...
	public:
		...
		// -------------------------------------------------------------------------------
		// @brief	バックバッファと深度バッファを新しい大きさで作り直す
		//
		//	ウィンドウのサイズ変更・最大化に追従するために呼ぶ
		//	同じ大きさなら何もしないため、毎フレーム呼んでも構わない
		//
		//	内部で必ずGPUの完了を待つ
		//	待たずに作り直すと、実行中のコマンドが消えたバックバッファを参照してしまう
		//
		// @param[in]	_width	新しい横幅(ピクセル)
		// @param[in]	_height	新しい縦幅(ピクセル)
		// @retval	true	この呼び出しのあと、その大きさで描画できる
		// -------------------------------------------------------------------------------
		bool Resize(uint32_t _width, uint32_t _height);

		// -------------------------------------------------------------------------------
		// @brief	デバッグレイヤーが溜めたメッセージをログへ出力して捨てる
		//
		//	D3D12のエラーはOutputDebugStringにしか出ないため、
		//	コンソールログだけを見ていると原因が分からない
		//	デバッグビルドでのみ動作し、リリースでは何もしない
		// -------------------------------------------------------------------------------
