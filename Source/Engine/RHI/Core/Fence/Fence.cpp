...
//		初期化処理を行う
// -------------------------------------------------------------------------------
bool RHI::Fence::Init(ID3D12Device* _pDevice)
{
	if (_pDevice == nullptr) 
	{ return false; }
...
//		シグナルを発行し、完了フェンス値を返す
// -------------------------------------------------------------------------------
UINT64 RHI::Fence::Signal(ID3D12CommandQueue* _pQueue)
{
	if (_pQueue == nullptr) 
	{ return 0; }
...
// -------------------------------------------------------------------------------
void RHI::Fence::WaitForValue(UINT64 _value)
{
	// すでに完了していれば即座に戻る
...
//		シグナル状態になるまでずっと待機
// -------------------------------------------------------------------------------
void RHI::Fence::Sync(ID3D12CommandQueue* _pQueue)
{
	if (_pQueue == nullptr) 
	{ return; }
