...
namespace {
		...
		const std::wstring& _modelDir)
	{
		...
		{
			const auto texType = static_cast<aiTextureType>(type);
			const unsigned int count = _pSrc->GetTextureCount(texType);
		...
		auto resolveTexPath = [&](std::initializer_list<aiTextureType> _candidaes)->std::wstring
			{
				for (const aiTextureType type : _candidaes)
				{
					...
					// 埋め込みテクスチャ（"*0"形式）はパスベースでは扱えないので弾く
					...
					// 3) ファイル名だけにして modelDir 直下を探す（サブフォルダ指定ズレの保険）
					auto byName = std::filesystem::path(_modelDir) / p.filename();
					if (std::filesystem::exists(byName))
					{
						ELOG("Texture resolved by filename fallback: %ls", byName.c_str());
						return byName.wstring();
					}

					ELOG("Texture not resolved type = %d raw[%s]", static_cast<int>(type), raw.C_Str());
				}
				return L"";
			};
		...
		_dst.DiffuseMap = resolveTexPath({ aiTextureType_BASE_COLOR,   // PBR/glTF
											 aiTextureType_DIFFUSE,      // 旧来 Phong
											 aiTextureType_EMISSIVE });  // 最後の保険
		_dst.NormalMap = resolveTexPath({ aiTextureType_NORMALS,
											 aiTextureType_HEIGHT });    // OBJ の bump はここに来る
