...
public:
	...
	// -------------------------------------------------------------------------------
	// @brief	GameObjectを「破棄せずに」管理下から切り離し、所有権を呼び出し側へ渡す
	//
	//	Removeとの違い
	//		Remove	… シーンから消し、実体も破棄する（元に戻せない）
	//		Detach	… シーンから消すが、実体は生かしたまま呼び出し側が預かる
	//
	//	エディタのUndoのために用意している
	//	「削除を取り消す」ときに同じオブジェクトをそのまま戻せるため、
	//	IDもコンポーネントの状態も完全に元通りになる
	//	作り直す方式ではポインタが変わり、コンポーネントの初期化状態も失われる
	//
	//	注意 :
	//		Update / Submit のループ中に呼んではいけない（その場でリストを書き換えるため）
	//		エディタはUIの組み立て中、つまりループの外から呼んでいる
	//
	// @param[in]	_pObject	切り離すGameObject
	// @return	切り離したGameObject（所有権つき）。見つからなければnullptr
	// -------------------------------------------------------------------------------
	...
	// -------------------------------------------------------------------------------
	void Submit(RenderQueue* _pQueue);
	...
	// -------------------------------------------------------------------------------
	// @brief	管理しているGameObjectの一覧を返す（読み取り専用）
	//
	//	エディタのヒエラルキーが、実際のシーンの中身をそのまま表示するために使う
	//	所有権はManagerが持ち続けるため、呼び出し側は参照するだけにする
	// -------------------------------------------------------------------------------
